...
def generate_batches(dataset, start_page_id, end_page_id, batch_ratio):
    images = []
    page_wh_list = []
    for index in tqdm(range(len(dataset))):
        if start_page_id <= index <= end_page_id:
            ...
            if len(images) == batch_ratio:
                yield images, page_wh_list
                images = []
                page_wh_list = []
...
def doc_analyze(
    ...
    if batch_analyze:
        ...
        for images, page_wh_list in generate_batches(dataset, start_page_id, end_page_id, batch_ratio):
            analyze_result.extend(batch_model(images))
            page_wh_list_result.extend(page_wh_list)
        
        for index in range(len(dataset)):
            if start_page_id <= index <= end_page_id:
                result = analyze_result.pop(0)
                page_width, page_height = page_wh_list_result.pop(0)
            else:
                result = []
                page_height = 0
                page_width = 0
